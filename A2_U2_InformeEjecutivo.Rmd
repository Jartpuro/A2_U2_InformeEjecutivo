...
required_packages <- c("paqueteMODELOS", "leaflet", "ggplot2", "effects", "plotly", "leaflet.extras", "dplyr", "lmtest")
...
cor_matrix <- cor(vivienda_casa[, c("areaconst", "estrato", "habitaciones", "parqueaderos", "banios")])
cor_matrix
...
<p> El gráfico de dispersión muestra la relación entre el precio y el área construida de apartamentos en la Zona Norte. Se observa una tendencia positiva, es decir, que a medida que aumenta el área construida, también aumenta el precio.

Coeficiente de correlación:El coeficiente de correlación de Pearson es de 0.8, lo que indica una fuerte correlación positiva entre las dos variables. Esto significa que existe una relación lineal significativa entre el precio y el área construida.

Interpretación del coeficiente:

Valor: Un valor de 0.8 indica que el 64% de la variación en el precio de los apartamentos puede explicarse por la variación en el área construida. El 36% restante se explica por otros factores no considerados en el gráfico.

Significancia: El coeficiente de correlación es estadísticamente significativo, lo que significa que es poco probable que se haya obtenido por casualidad.</p>
```{r , warning=FALSE, message=FALSE, echo=FALSE}
