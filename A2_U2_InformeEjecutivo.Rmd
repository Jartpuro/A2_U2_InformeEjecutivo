...
<h2>Base 3 Casas Zona Oriente</h2>
...
<h2> Base 5 Casas Zona Centro</h2>
...
<p><b>Estadísticas de ajuste del modelo/<b>: La salida obtenida también incluye  el valor R2, que es la proporción de la variabilidad en el precio de la vivienda que es explicada por el modelo. En este caso, R2 es aproximadamente 0.7206364, lo que significa que alrededor del 72.06% de la variabilidad en el precio de la vivienda puede ser explicada por las variables independientes incluidas en el modelo, como las descritas anteriormente. </p>
...
# Crear un dataframe con las características de las viviendas
viviendas_nuevas <- data.frame(
  Tipo = c("Casa"),
  areaconst = c(200),
  parqueaderos = c(1),
  banios = c(2),
  habitaciones = c(4),
  estrato = c(4,5),  # Se incluyen los estratos el tipo de vivienda
  zona = c("Norte"),
  credito_preaprobado = c(350)
)
...
vivienda_solicitada <- data.frame(
  tipo = "Casa",
  areaconst = 200,
  parqueaderos = 1,
  banios = 2,
  habitaciones = 4,
  estrato = c(4, 5),  # Asumiendo que el estrato puede ser 4 o 5
  zona = "Norte",
...
# Filtrar ofertas potenciales que cumplen con las características de la vivienda solicitada
ofertas_potenciales <- subset(vivienda,
                               tipo == vivienda_solicitada$tipo &
                                 areaconst == vivienda_solicitada$areaconst &
                                 parqueaderos == vivienda_solicitada$parqueaderos &
                                 banios == vivienda_solicitada$banios &
                                 habitaciones == vivienda_solicitada$habitaciones &
                                 estrato %in% vivienda_solicitada$estrato &
                                 zona == vivienda_solicitada$zona &
                                 preciom <= vivienda_solicitada$credito_preaprobado)

# Filtrar observaciones sin valores faltantes
ofertas_potenciales <- na.omit(ofertas_potenciales)

# Verificar el número de observaciones en ofertas_potenciales
num_observaciones <- nrow(ofertas_potenciales)

if (num_observaciones >= 5) {
  # Seleccionar aleatoriamente 5 observaciones
  ofertas_seleccionadas <- ofertas_potenciales %>%
    sample_n(5, replace = TRUE)
  
  # Visualizar ofertas potenciales en un mapa
  mapa <- ggplot() +
    geom_point(data = ofertas_seleccionadas, aes(x = longitud, y = latitud), color = "blue") +
    labs(x = "Longitud", y = "Latitud", title = "Ofertas Potenciales") +
    theme_minimal()
  
  # Imprimir mapa
  print(mapa)
} else {
  print("No hay suficientes observaciones para seleccionar 5 ofertas potenciales.")
}
...
<p>Base 1 Apartamentos Zona Norte</p>
```{r , warning=FALSE, message=FALSE, echo=FALSE}
...
<p>Base 2 Apartamentos Zona Sur</p>
```{r , warning=FALSE, message=FALSE, echo=FALSE}
...
<p>Base 2 Apartamentos Zona Oeste</p>
```{r , warning=FALSE, message=FALSE, echo=FALSE}
...
<p>Base 2 Apartamentos Zona Oriente</p>
```{r , warning=FALSE, message=FALSE, echo=FALSE}
...
<p>Base 2 Apartamentos Zona Centro</p>
```{r , warning=FALSE, message=FALSE, echo=FALSE}
